...
export async function GET(req: NextRequest) {
  ...
  if (type === 'meal') {
    const recs = await prisma.mealRecord.findMany({
      ...
      select: { residentId: true, mealType: true, mainDish: true, sideDish: true },
    })
    const mealMap = new Map<number, Record<string, { mainDish: number | null; sideDish: number | null }>>()
    recs.forEach(r => {
      if (!mealMap.has(r.residentId)) mealMap.set(r.residentId, {})
      mealMap.get(r.residentId)![r.mealType] = { mainDish: r.mainDish, sideDish: r.sideDish }
    })
  ...
  if (type === 'vital') {
    const recs = await prisma.vitalRecord.findMany({
      where: { recordedAt: { gte: dateStart, lte: dateEnd } },
      select: { residentId: true, systolic: true, diastolic: true, pulse: true, temperature: true, spo2: true },
  ...
  if (type === 'medication') {
    const recs = await prisma.medicationRecord.findMany({
      where: { recordedAt: { gte: dateStart, lte: dateEnd } },
      select: {
        residentId: true, beforeBreakfast: true, afterBreakfast: true,
  ...
  if (type === 'night-patrol') {
    const recs = await prisma.nightPatrolRecord.findMany({
      where: { recordedAt: { gte: dateStart, lte: dateEnd } },
      select: { residentId: true, patrolTime: true, status: true },
      orderBy: { recordedAt: 'asc' },
    })
  ...
  if (type === 'comment') {
    const recs = await prisma.commentRecord.findMany({
      where: { recordedAt: { gte: dateStart, lte: dateEnd } },
      select: { residentId: true, category: true, content: true },
    })
